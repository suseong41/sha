...
```
규칙 25종(HIGH 11) · 테스트 762개 · 정상 코퍼스 21쪽 · 실전 측정 도구(tools/measure.sh) · 퍼징 5,600만 케이스 무결
...
**확인은 헤드리스 Chrome 으로**(jsdom 은 CSP 를 강제하지 않는다) — 두 페이지 위반 **0건**, 카드가 그려짐 = 스크립트가 돌았다는 증거.
