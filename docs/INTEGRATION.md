...
작성 2026-09-16 · SHA 는 Go 1.27 · 이미지 `scratch` 기반
...
| `mem_limit` · `pids_limit` | **시작값이다. 부하를 걸어 잰 값이 아니다.** SHA 는 동시 요청 수를 스스로 제한하지 않는다(요청당 응답 최대 5MB). 동시 요청의 상한은 nginx 의 `limit_req` 와 이 메모리 제한이 함께 정한다 |
