...
sarif/         SARIF 2.1.0 출력 — CLI 의 -sarif. 버전은 인자로 받는다(version 을 import 하지 않음)
...
등급은 **출력 계약**이다(종료 코드 · API 의 `severity`). `TestEveryRuleSeverity` 가 25종 전부를 못으로 박는다(**정정 2026-09-22**: 71교시에 25종이라 적었으나 실제로는 22종이었다 — 표본이 안 띄운 2종과 표에서 빠진 1종을 74교시에 채웠다) — 바꾸려면 재서 근거를 남기고 그 표를 함께 고친다.
...
```
규칙 25종(HIGH 11) · 테스트 787개 · 정상 코퍼스 21쪽 · 실전 측정 도구(tools/measure.sh) · 퍼징 5,600만 케이스 무결
...
| **응답 헤더 규칙** | **2026-09-22 측정 후 만들지 않음(사용자 결정 — "실측도 적고 크게 유효하지 않다").** 실제 50곳에서 잰 결과 후보 대부분이 **"없음" 계열**이라 `sri-missing` 과 같은 모양이었다(CSP 없음 57% · 클릭재킹 없음 58% · nosniff 없음 58% · base-uri 없음 30%). 조합도 얇다 — `base-uri 없음 ∧ <base>` **0곳**(`<base>` 자체가 1곳) · `Secure 없는 쿠키 ∧ 비밀번호 칸` 은 **코퍼스에 비밀번호 칸이 0곳이라 오탐을 잴 수 없음**(§4-3 위반). 살아남은 것은 `csp-unsafe-inline` 하나인데 10곳/50, **위키백과 4개 언어판이 한 운영자**라 중복 제거하면 7/47(15%). 범위 문제도 있다 — 우리 약속은 "HTML에서 악성 행위"이고 헤더 점검은 *"이 사이트가 잘 굳혀져 있는가"* 라는 다른 질문이다(섞으면 C++ 전신의 실패를 반복한다). **다시 볼 조건**: SHA 를 "HTML 탐지기"가 아니라 "페이지 점검 도구"로 정체를 바꿀 때 — 그때는 헤더가 곁가지가 아니라 중심이다 |
...
**74교시** — 규칙 25종에 **설명**(왜 위험한가 · 어떻게 고치나) · 응답의 `rules` 맵 · 화면에서 펼치기. **등급만 말하면 아무도 못 고친다.**
설명을 규칙 구조체가 아니라 **한 표**에 모았다 — 문장이 구현 파일에 흩어지지 않고, 표 자체가 읽을 수 있는 문서가 되며, "빠진 규칙이 있는가"를 테스트가 한 번에 묻는다.
응답 모양은 재서 정했다: **한 페이지에 나오는 코드는 최대 5종**(정상 0~3종) → 발견마다 붙이면 상한이 없고, 코드마다 한 번이면 **1.2KB 안쪽**. 빈 맵도 `{}` 로 보낸다(nil 은 `null` → 받는 쪽 `Object.keys` 가 터진다).
**테스트를 쓰다가 더 큰 것이 나왔다** — `TestEveryRuleSeverity` 가 **22종만** 박고 있었다(`resource-ip-literal` 은 표에 없어 조용히 지나갔고, `mixed-script-host`·`phishing-interstitial` 은 표본이 안 띄웠다). 표본에 두 줄을 더해 25종을 띄우고 **반대 방향**(표에 없는 코드가 발동하면 실패)을 넣었다.
변이 7건 전부 잡힘. 그중 **맵 초기화 빼기는 엉뚱한 테스트를 빨갛게 했다** — nil 맵 대입은 **패닉**이라 바이너리가 죽어 뒤 테스트가 보고되지 않는다. 내 테스트가 실제로 지키는지는 **그것만 따로 돌려** 확인했다. **775 테스트.**
화면은 발견 행을 누르면 설명이 펼쳐진다(Enter·Space 포함). 그리는 쪽은 `data.rules?.[code]` 로 읽는다 — **정적 파일과 이미지는 따로 배포**되므로 페이지가 먼저 올라가면 옛 서버가 `rules` 없이 응답한다.
**작업 방식 변경(사용자 결정)**: my_homepage 의 **HTML·JS·CSS 는 내가 직접 쓴다**(주석 한 줄 이내, 문서 참조 금지). Go 구현은 여전히 사용자가 친다.

**75교시** — `-sarif` (SARIF 2.1.0). **남이 정한 형식은 남의 검증기로 봐야 한다.**
등급은 `HIGH→error` · `MEDIUM→warning` · `LOW·INFO→note`, `security-severity` 는 8.0/5.0/2.0 이고 **INFO 는 비운다**(보안 경보가 아니다). `rules` 에는 나온 규칙만 담고, 74교시의 설명이 `fullDescription`·`help` 로 그대로 들어간다. **참고는 결과가 아니라 `toolExecutionNotifications`** 로(§3). 종료 코드는 그대로 — `-sarif` 는 형식만 바꾼다. `sarif` 는 `version` 을 import 하지 않고 **인자로 받는다**(63교시).
검증은 **공식 스키마**(OASIS SARIF 2.1.0)로 했다 — 악성·정상·참고만 있는 SPA 셋 다 통과, **대조군**(필수 필드 제거·version 변조·message 제거)은 전부 실패. 변이 단위 7 · CLI 2 전부 잡힘. **787 테스트.**
**오늘의 핵심은 결함 하나가 층을 가른 것이다.** `json:"physicalLocation` (닫는 따옴표 누락) →
| 층 | 잡았나 | 왜 |
|---|---|---|
| 컴파일러 | ✗ | 태그는 그냥 문자열 |
| `go vet` | **✓** | 태그의 **모양**을 본다 |
