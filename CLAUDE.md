...
go test ./...           # 557개 (서브테스트 포함) · 주입 테스트 포함 약 6초
...
```
규칙 22종 · 테스트 557개 · 정상 코퍼스 17쪽 · 퍼징 5,600만 케이스 무결
...
1. **`punycode` · 혼합 스크립트** — 선행 조건(정상 IDN 표본)이 아직 없다. 코퍼스에 IDN 페이지를 더 넣고 시작한다.
   39교시의 `unicode.Scripts` 판정을 그대로 쓸 수 있다.
