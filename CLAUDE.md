...
> 26~46교시의 상세는 **DISCUSSION.md 12절**에 있다. 아래 요약은 압축 후 방향을 잃지 않기 위한 것이다.
...
1. **`fetcher` 패키지** — SSRF 방어 수집기. `http`/`https` 만 · 해석한 IP 검사(사설 10/8·172.16/12·192.168/16,
   루프백, 링크로컬 169.254/16, 멀티캐스트 차단) · **리다이렉트마다 재검사** · 타임아웃 10초 · 최대 5MB ·
   리다이렉트 3회 · 쿠키/인증 헤더 금지. **DNS 재바인딩 방어**: 검증한 IP 로 직접 접속하고 `Host` 헤더만 도메인으로
   (`http.Transport.DialContext`). 테스트로 검증 가능한 층이라 여기부터 한다.
