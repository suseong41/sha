...
```
규칙 23종 · 테스트 619개 · 정상 코퍼스 21쪽 · 실전 측정 도구(tools/measure.sh) · 퍼징 5,600만 케이스 무결
원본 97개 항목 이식 완료 (이식 18 · 조합 재료 3 · 버림 76)
실전 43쪽 측정: 393건 → 115건 · HIGH 0건 | 커버리지 main 98.5% · scanner 99.0% · tokenizer 95.1%
패키지: tokenizer · scanner · fetcher(47~48교시 신설, SSRF 방어)
```

> 26~48교시의 상세는 **DISCUSSION.md 12절**에 있다. 아래 요약은 압축 후 방향을 잃지 않기 위한 것이다.
...
1. ~~**`fetcher` 패키지** — 주소 검증 + DNS 재바인딩 방어~~ → **47·48교시에서 완료.**
   `fetcher/addr.go`(`blockedReason`) · `fetcher/fetch.go`(`Fetcher.dialChecked`). 변이 검사 26/26.
   **남은 것 → 49교시**: 타임아웃 10초 · 최대 5MB · 리다이렉트 3회 · `CheckRedirect` 로 scheme 재검사 ·
   쿠키/인증 헤더 금지. (리다이렉트 **주소** 재검사는 홉마다 `DialContext` 가 불려 이미 된다.)
2. **출력 이스케이프** — `Evidence` 에 **원본 마크업이 그대로** 들어 있다(`<img onerror=…>`).
   이스케이프 없이 렌더링하면 **우리 스캐너가 우리 사이트에 XSS 를 심는다.** CSP 도 함께.
3. **웹 계층** — 그 위에 얹는다.
