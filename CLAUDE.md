...
go test ./...           # 626개 (서브테스트 포함) · 주입 테스트 포함 약 10초
...
```
규칙 23종 · 테스트 626개 · 정상 코퍼스 21쪽 · 실전 측정 도구(tools/measure.sh) · 퍼징 5,600만 케이스 무결
원본 97개 항목 이식 완료 (이식 18 · 조합 재료 3 · 버림 76)
실전 43쪽 측정: 393건 → 115건 · HIGH 0건 | 커버리지 main 98.5% · scanner 99.0% · tokenizer 95.1%
패키지: tokenizer · scanner · fetcher(47~49교시 신설, SSRF 방어 + 자원 상한, 변이 검사 37/37)
```

> 26~49교시의 상세는 **DISCUSSION.md 12절**에 있다. 아래 요약은 압축 후 방향을 잃지 않기 위한 것이다.
